...
#error "Please define reduction type."
    ...
    vector<Root_> psitable1 =
        parameters.gpu_root_of_unity_table_generator(parameters.n1_based_root_of_unity_table);
    ...
    GPU_4STEP_NTT(Output_Datas, Input_Datas, psitable_device1, psitable_device2, W_Table_device,
                  test_modulus, cfg_intt, BATCH, 1);

    GPU_Transpose(Input_Datas, Output_Datas, parameters.n1, parameters.n2, parameters.logn, BATCH);

    vector<Data> Output_Host(parameters.n * BATCH);
    cudaMemcpy(Output_Host.data(), Output_Datas, parameters.n * BATCH * sizeof(Data),
