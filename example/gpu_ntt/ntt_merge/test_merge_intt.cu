...
#error "Please define reduction type."
    ...
    THROW_IF_CUDA_ERROR(cudaMalloc(&InOut_Datas, BATCH * parameters.n * sizeof(Data)));

    for(int j = 0; j < BATCH; j++)
    {
        THROW_IF_CUDA_ERROR(cudaMemcpy(InOut_Datas + (parameters.n * j), input1[j].data(),
    ...
    THROW_IF_CUDA_ERROR(
        cudaMalloc(&Inverse_Omega_Table_Device, parameters.root_of_unity_size * sizeof(Root)));

    vector<Root_> inverse_omega_table =
        parameters.gpu_root_of_unity_table_generator(parameters.inverse_root_of_unity_table);
    THROW_IF_CUDA_ERROR(cudaMemcpy(Inverse_Omega_Table_Device, inverse_omega_table.data(),
    ...
    ntt_rns_configuration cfg_intt = {.n_power = LOGN,
                                      ...
                                      .stream = 0};
    GPU_NTT_Inplace(InOut_Datas, Inverse_Omega_Table_Device, test_modulus, cfg_intt, BATCH, 1);
    ...
    THROW_IF_CUDA_ERROR(cudaMemcpy(Output_Host, InOut_Datas, BATCH * parameters.n * sizeof(Data),
    ...
    for(int i = 0; i < BATCH; i++)
    {
        check = check_result(Output_Host + (i * parameters.n), ntt_result[i].data(), parameters.n);
