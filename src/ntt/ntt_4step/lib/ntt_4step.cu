...
{
    Transpose_Batch<<<dim3(col >> 4, row >> 4, batch_size), dim3(16, 16)>>>(input, output, row, col,
                                                                            n_power);
                                      ...
                                      int index2, int n_power, int mod_count)
{
    ...
    CooleyTukeyUnit(sharedmemorys[(idx_y << 1) + global_index1][in_shared_address],
                    sharedmemorys[(idx_y << 1) + global_index1][in_shared_address + t],
                    n1_root_of_unity_table[(global_index2 >> t_)], q_thread);
    CooleyTukeyUnit(sharedmemorys[(idx_y << 1) + global_index1 + 16][in_shared_address],
                                      ...
                                      int index2, int index3, int n_power, int mod_count)
{
    ...
    CooleyTukeyUnit(sharedmemorys[idx_y][in_shared_address],
                    sharedmemorys[idx_y][in_shared_address + t],
                    n1_root_of_unity_table[(idx_x >> t_)], q_thread);
    CooleyTukeyUnit(sharedmemorys[idx_y + 8][in_shared_address],
                                      ...
                                      int index2, int index3, int n_power, int mod_count)
{
    ...
    CooleyTukeyUnit(sharedmemorys[shr_in][in_shared_address],
                    sharedmemorys[shr_in][in_shared_address + t],
                    n1_root_of_unity_table[(shr_address >> t_)], q_thread);
    CooleyTukeyUnit(sharedmemorys[shr_in + 4][in_shared_address],
                                      ...
                                      int index2, int index3, int n_power, int mod_count)
{
    ...
    CooleyTukeyUnit(sharedmemorys[shr_in][in_shared_address],
                    sharedmemorys[shr_in][in_shared_address + t],
                    n1_root_of_unity_table[(shr_address >> t_)], q_thread);
    CooleyTukeyUnit(sharedmemorys[shr_in + 2][in_shared_address],
                                            ...
                                            int n_power, int mod_count)
{
    ...
    {
        CooleyTukeyUnit(sharedmemorys[in_shared_address], sharedmemorys[in_shared_address + t],
...
#pragma unroll
    ...
    {
        CooleyTukeyUnit(sharedmemorys[shrd_address], sharedmemorys[shrd_address + t],
                                      ...
                                      int index2, int n_power, int mod_count)
{
    ...
    GentlemanSandeUnit(sharedmemorys[(idx_y << 1) + global_index1][in_shared_address],
                       sharedmemorys[(idx_y << 1) + global_index1][in_shared_address + t],
                       n1_root_of_unity_table[(global_index2 >> t_)], q_thread);
    GentlemanSandeUnit(sharedmemorys[(idx_y << 1) + global_index1 + 16][in_shared_address],
                       sharedmemorys[(idx_y << 1) + global_index1 + 16][in_shared_address + t],
                       n1_root_of_unity_table[(global_index2 >> t_)], q_thread);
    __syncthreads();
                                      ...
                                      int index2, int index3, int n_power, int mod_count)
{
    ...
    GentlemanSandeUnit(sharedmemorys[idx_y][in_shared_address],
                       sharedmemorys[idx_y][in_shared_address + t],
                       n1_root_of_unity_table[(idx_x >> t_)], q_thread);
    GentlemanSandeUnit(sharedmemorys[idx_y + 8][in_shared_address],
                       sharedmemorys[idx_y + 8][in_shared_address + t],
                       n1_root_of_unity_table[(idx_x >> t_)], q_thread);
    __syncthreads();
                                      ...
                                      int index2, int index3, int n_power, int mod_count)
{
    ...
    GentlemanSandeUnit(sharedmemorys[shr_in][in_shared_address],
                       sharedmemorys[shr_in][in_shared_address + t],
                       n1_root_of_unity_table[(shr_address >> t_)], q_thread);
    GentlemanSandeUnit(sharedmemorys[shr_in + 4][in_shared_address],
                       sharedmemorys[shr_in + 4][in_shared_address + t],
                       n1_root_of_unity_table[(shr_address >> t_)], q_thread);
    __syncthreads();
                                      ...
                                      int index2, int index3, int n_power, int mod_count)
{
    ...
    GentlemanSandeUnit(sharedmemorys[shr_in][in_shared_address],
                       sharedmemorys[shr_in][in_shared_address + t],
                       n1_root_of_unity_table[(shr_address >> t_)], q_thread);
    GentlemanSandeUnit(sharedmemorys[shr_in + 2][in_shared_address],
                       sharedmemorys[shr_in + 2][in_shared_address + t],
                       n1_root_of_unity_table[(shr_address >> t_)], q_thread);
    __syncthreads();
...
#pragma unroll
    for(int loop_dep = 0; loop_dep < LOOP; loop_dep++)
    {
        GentlemanSandeUnit(sharedmemorys[shrd_address], sharedmemorys[shrd_address + t],
                           n2_root_of_unity_table[(local_idx >> t_2)], q_thread);
    ...
    Data temp1 = VALUE_GPU::mult(sharedmemorys[shrd_address], inverse[0], q_thread);
                                            ...
                                            int poly_n_power, int mod_count)
{
    ...
    GentlemanSandeUnit(sharedmemorys[in_shared_address], sharedmemorys[in_shared_address + t],
                       n2_root_of_unity_table[(global_addresss >> t_2)], q_thread);
    __syncthreads();
            ...
            switch(cfg.n_power)
            {
                ...
                case 18:
                    ...
                    FourStepPartialForwardCore2<<<dim3(16, 32, batch_size), 256>>>(
                        device_out, n2_root_of_unity_table, modulus, 13, 18, mod_count);
                    THROW_IF_CUDA_ERROR(cudaGetLastError());
                    break;
                case 19:
