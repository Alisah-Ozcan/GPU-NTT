...
std::vector<Data> schoolbook_poly_multiplication(std::vector<Data> a, std::vector<Data> b,
                                                 Modulus modulus,
...
std::vector<Data> NTT_CPU::mult(std::vector<Data> &input1, std::vector<Data> &input2)
{
    ...
    while(m > 1)
    {
        ...
        for(int i = 0; i < h; i++)
        {
            ...
            for(int j = j1; j < (j2 + 1); j++)
            {
                ...
                output[j] = VALUE::add(U, V, parameters.modulus);
                output[j + t] = VALUE::sub(U, V, parameters.modulus);
                output[j + t] = VALUE::mult(output[j + t], S, parameters.modulus);
            }
