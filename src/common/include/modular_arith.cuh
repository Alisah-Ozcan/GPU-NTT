...
class BarrettOperations
{
   public:
    ...
    static __host__ Data exp(Data& base, Data& exponent, Modulus& modulus)
    {
        ...
        {
            result = barrett64_cpu::BarrettOperations::mult(result, result, modulus);
            if(((exponent >> i) & 1u))
            {
                result = barrett64_cpu::BarrettOperations::mult(result, base, modulus);
            }
        }
...
class GoldilocksOperations
{
   public:
    ...
    static __host__ Data mult(Data& input1, Data& input2, Modulus& modulus)
    {
        ...
        __uint128_t pre_result = (__uint128_t)(hiL * 4294967295) + (__uint128_t)lo;
    ...
    static __host__ Data exp(Data& base, Data& exponent, Modulus& modulus)
    {
        ...
        for(int i = modulus_bit - 1; i > -1; i--)
        {
            result = goldilocks64_cpu::GoldilocksOperations::mult(result, result, modulus);
            if(((exponent >> i) & 1u))
            {
                result = goldilocks64_cpu::GoldilocksOperations::mult(result, base, modulus);
            }
        }
...
class BarrettOperations
{
   private:
    class uint128_t
    {
       public:
        ...
        __device__ __forceinline__ uint128_t(const Data& input)
        {
            value.x = input;
            value.y = 0;
        }
        ...
        __device__ __forceinline__ uint128_t operator-(uint128_t& other)
        {
            ...
            asm("{\n\t"
                ...
                : "l"(value.y), "l"(value.x), "l"(other.value.y), "l"(other.value.x));
   ...
   public:
    ...
    static __device__ uint128_t mult128(const unsigned long long& a, const unsigned long long& b)
    {
        uint128_t result;
    ...
    static __device__ Data mult(Data& input1, Data& input2, Modulus& modulus)
    {
        ...
        return (z.value.x >= modulus.value) ? (z.value.x -= modulus.value) : (z.value.x);
    }

    // Barrett Reduction for 64 bit
    // result = input1 % modulus
    static __device__ Data reduce(Data& input1, Modulus& modulus)
    {
        uint128_t z(input1);
...
__device__ __forceinline__ uint128_t operator-(uint128_t& input1, const uint128_t& input2)
{
    uint128_t result;
...
__device__ __forceinline__ uint128_t operator-(uint128_t& input1, const Data& input2)
{
    uint128_t result;
...
__device__ __forceinline__ void operator-=(uint128_t& input1, const uint128_t& input2)
{
    asm("{\n\t"
        "sub.cc.u64      %1, %3, %5;    \n\t"
        "subc.u64        %0, %2, %4;    \n\t"
...
__device__ __forceinline__ void operator-=(uint128_t& input1, const Data& input2)
{
    asm("{\n\t"
        "sub.cc.u64      %1, %3, %5;    \n\t"
        "subc.u64        %0, %2, %4;    \n\t"
...
__device__ __forceinline__ bool operator>=(uint128_t& input1, const uint128_t& input2)
{
...
__device__ __forceinline__ bool operator>=(uint128_t& input1, const Data& input2)
{
    if(input1.y > 0)
        return true;
...
struct GoldilocksOperations
{
   public:
    // Modular Addition for 64 bit
    // result = (input1 + input2) % modulus
    __device__ __forceinline__ static Data add(Data& input1, Data& input2, Modulus& modulus)
    {
    ...
    // Modular Substraction for 64 bit
    // result = (input1 - input2) % modulus
    __device__ __forceinline__ static Data sub(Data& input1, Data& input2, Modulus& modulus)
    {
    ...
    // Modular Multiplication for 64 bit
    // result = (input1 * input2) % modulus
    __device__ __forceinline__ static Data mult(Data& input1, Data& input2, Modulus& modulus)
    {
        // --------------------- //
        // Authors: Alisah Ozcan
        // --------------------- //
...
class PlantardOperations
{
   ...
   public:
    ...
    // Modular Multiplication for 64 bit
    // result = (input1 * input2) % modulus
    static __device__ Data mult(Data& input1, ulonglong2& input2, Modulus& modulus)
    {
        // --------------------- //
        // Authors: Alisah Ozcan
        // --------------------- //
