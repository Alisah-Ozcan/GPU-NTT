...
template <typename T> Modulus<T> NTTParameters<T>::modulus_pool()
{
    ...
    {
        static Data32 primes[] = {268460033, 268582913, 268664833, 268369921,
                                  269221889, 269221889, 270532609, 270532609,
                                  270532609, 377487361, 377487361, 469762049,
                                  469762049};
...
template <typename T> T NTTParameters<T>::omega_pool()
{
    ...
    if constexpr (std::is_same<T, Data32>::value)
    {
        static Data32 W[] = {36747374,  249229369, 4092529, 175218169, 10653696,
                             238764304, 240100,    23104,   179776,    19321,
                             38809,     1600,      169};
...
template <typename T> T NTTParameters<T>::psi_pool()
{
    ...
    {
        static Data64 PSI[] = {238394956950829, 54612008597396, 8242615629351,
...
template <typename T> std::vector<int> NTTParameters4Step<T>::matrix_dimention()
{
    customAssert(12 <= logn <= 24, "LOGN should be in range 12 to 24.");
...
{
    T exp_n1 = int(n / n1);
    T small_root_of_unity_n1 = OPERATOR<T>::exp(root_of_unity, exp_n1, modulus);
    ...
    T exp_n2 = int(n / n2);
    T small_root_of_unity_n2 = OPERATOR<T>::exp(root_of_unity, exp_n2, modulus);
...
template <typename T> void NTTParameters4Step<T>::TW_forward_table_generator()
{
    int lg = log2(n1);
    for (int i = 0; i < n1; i++)
    {
...
void NTTParameters4Step<T>::small_inverse_root_of_unity_table_generator()
{
    ...
    for (int i = 1; i < (n1 >> 1); i++)
    {
        T exp = OPERATOR<T>::mult(n1_based_inverse_root_of_unity_table[i - 1],
                                  small_root_of_unity_n1, modulus);
    ...
    for (int i = 1; i < (n2 >> 1); i++)
    {
        T exp = OPERATOR<T>::mult(n2_based_inverse_root_of_unity_table[i - 1],
                                  small_root_of_unity_n2, modulus);
        n2_based_inverse_root_of_unity_table.push_back(exp);
    }
}

template <typename T> void NTTParameters4Step<T>::TW_inverse_table_generator()
{
    int lg = log2(n2);
    for (int i = 0; i < n1; i++)
    {
...
template <typename T>
std::vector<Root<T>>
NTTParameters4Step<T>::gpu_root_of_unity_table_generator(std::vector<T> table)
{
    // Taking Bitreverse order of root of unity table
