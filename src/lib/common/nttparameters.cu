...
std::vector<Root_>
NTTParameters::gpu_root_of_unity_table_generator(std::vector<Data> table)
{
    ...
    for (int i = 0; i < n1; i++)
    {
        for (int j = 0; j < n2; j++)
        {
            ...
            W_root_of_unity_table.push_back(
                VALUE::exp(root_of_unity, index, modulus));
        }
    }
}
    ...
    for (int i = 0; i < n1; i++)
    {
        for (int j = 0; j < n2; j++)
        {
            ...
            W_inverse_root_of_unity_table.push_back(
                VALUE::exp(inverse_root_of_unity, index, modulus));
        }
    }
}
...
std::vector<Root_>
NTTParameters4Step::gpu_root_of_unity_table_generator(std::vector<Data> table)
{
    // Taking Bitreverse order of root of unity table
