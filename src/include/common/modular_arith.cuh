...
{ // It does not work  modulus higher than 62 bit.
    ...
    class BarrettOperations
    {
      public:
        ...
        // Modular Reduction
        static __host__ Data reduce(Data& input1, Modulus& modulus)
        {
            __uint128_t mult = (__uint128_t) input1;
...
{ // It does not work  modulus higher than 62 bit.
    ...
    class GoldilocksOperations
    {
      public:
        ...
        {
            __uint128_t dif =
                (__uint128_t) input1 + (__uint128_t) modulus.value;
            dif = dif - input2;
            dif = (dif >= modulus.value) ? (dif - modulus.value) : dif;
        ...
        {
            Data index = modulus.value - 2;
            return goldilocks64_cpu::GoldilocksOperations::exp(input, index,
                                                               modulus);
        }
    };
...
namespace barrett64_gpu
{
    class BarrettOperations
    {
      ...
      public:
        ...
        // Modular Multiplication for 64 bit
        // result = (input1 * input2) % modulus
        static __device__ Data mult(Data& input1, Data& input2,
                                    Modulus& modulus)
        {
        ...
        // Barrett Reduction for 64 bit
        // result = input1 % modulus
        static __device__ Data reduce_forced(Data& input1, Modulus& modulus)
        {
            Data z = input1;
            while (z >= modulus.value)
            {
                z = reduce(z, modulus);
            }
      ...
      public:
                                    ...
                                    Modulus& modulus)
        {
            ...
            asm("{\n\t"
                ...
                "}"
                : "=l"(result)
                : "l"(input1), "l"(input2.x), "l"(input2.y),
                  "l"(modulus.value));
